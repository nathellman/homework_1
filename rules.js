...
function begin_play(){
	...
	if (started) window.alert("Already started, please reset_play to start again");
		...
		turn.innerHTML = "Turn starts with " + 'X'.bold();
	}
...
function play() {
	...
	var inputsPossible = new Array('A1', 'A2', 'A3', 'B1', 'B2', 'B3', 'C1', 'C2', 'C3')
	var enteredInput = document.getElementById('move_text_id');
	console.log(enteredInput.value);
	var i;
	for (i = 0; i < inputsPossible.length; i++){
		if (enteredInput.value.match(inputsPossible[i])) {
			var there = true;
			break;
		}
		else var there = false;
	}
	if ((there == true) && (document.getElementById(enteredInput.value).innerHTML != enteredInput.value)) var filled = true;
	else var filled = false;
	if ((there == false) || (filled == true)) window.alert("Please enter a valid cell");
	else {
		console.log(enteredInput);
		if (turnNum % 2 == 0)
		{
			document.getElementById(enteredInput.value).innerHTML = 'X'
			var turn = document.getElementById('turn_info');
			turn.innerHTML = "Turn for: " + 'O'.bold();
		}
		else
		{
			document.getElementById(enteredInput.value).innerHTML = 'Y';
			var turn = document.getElementById('turn_info');
...
function checkwinner(){
	...
	//check columns
	for (var i = 0; i < 3; i++){
		if ((table[i].innerHTML == table[i+3].innerHTML) && (table[i].innerHTML == table[i+6].innerHTML)){
			window.alert("Player " + table[i].innerHTML + " wins");
			winner = true;
			reset_play();
		}
	}
	...
	//check rows
	for (var i = 0; i < 9; i+=3){
		if ((table[i].innerHTML == table[i+1].innerHTML) && (table[i].innerHTML == table[i+2].innerHTML)){
			window.alert("Player " + table[i].innerHTML + " wins");
			winner = true;
			reset_play();
		}
	...
	//check diagonal
	if ((table[0].innerHTML == table[4].innerHTML) && (table[0].innerHTML == table[8].innerHTML)){
		window.alert("Player " + table[0].innerHTML + " wins");
		winner = true;
		reset_play();
	}
	else if ((table[2].innerHTML == table[4].innerHTML) && (table[2].innerHTML == table[6].innerHTML)){
		window.alert("Player " + table[2].innerHTML + " wins");
		winner = true;
		reset_play();
	}
	...
	{
		if (table[i].innerHTML != table[i].id) filled++
	}
	if (winner == false && filled == 9) reset_play();
